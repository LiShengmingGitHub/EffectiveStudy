...
----------
## 1.Redis的作用
Redis是一个高性能的key-value存储系统，支持多种存储结构，并且支持持久化。如果想进一步全面的了解Redis的作用，可以看下郑毅整理的材料。

    注：Redis是一个内存数据库，相对于传统的DB存取速度自然更快，还有其他原因，接下来会重点分析
...
![redis-master-slave](picture-section/redis-master-slave.png)
...
![redis-master-slave](picture-section/redis-sentinel.png)
...
![redis-cluster](picture-section/redis-cluster.png)
...
![redis-cluster-final](picture-section/redis-cluster-final.png)
...
redis事务采用的是乐观锁，就是先处理，再真正去执行的时候去看有没有被修改过，没有的话，就去更新，否则报错。这个思想是不是有点似曾相识，是的，concurrenthashmap的set值时也是用的这个思想。还是那句话，虽然技术上感觉不搭边，但是思想上确实想通的。
    ...
    注：集群环境下，事务操作处理的key分布式在不同分片上，是不不支持的。
#### 2.6.2Redis分布式锁
